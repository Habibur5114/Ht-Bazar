...
@section('title') {{ $title ?? 'product Edit' }} @endsection
@section('content')
    <div class="mt-5">
        <main class="app-main">
                                        ...
                                        <div class="row">
                                                    ...
                                                    <select class="form-select form-control" name="ChildCategory_id">
                                                        ...
                                                        @foreach ($ChildCategorys as $ChildCategory)
                                                            <option value="{{ $ChildCategory->id }}"
                                                                {{ old('childcategory_id', $product->childcategory_id ?? '') == $ChildCategory->id ? 'selected' : '' }}>
                                                                {{ $ChildCategory->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                    ...
                                                    <select class="size form-control" name="size[]" multiple="multiple">
                                                        @foreach ($sizes as $size)
                                                            <option value="{{ $size->id }}"
                                                                {{ (is_array(old('size')) && in_array($size->id, old('size'))) || (is_array($product->size) && in_array($size->id, $product->size)) ? 'selected' : '' }}>
                                                                {{ $size->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                    ...
                                                    <select class="color form-select" name="color[]" multiple="multiple">
                                                        @foreach ($colors as $color)
                                                            <option value="{{ $color->id }}"
                                                              {{ (is_array(old('color')) && in_array($color->id, old('color'))) || (is_array($product->color) && in_array($color->id, $product->color)) ? 'selected' : '' }}>
                                                                {{ $color->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <div class="col-md-12">
                                                <div class="mb-3">
                                                    ...
                                                    <textarea name="description" class="form-control editor" placeholder="Enter description" style="height: 400px">{{ old('description', $product->description) }}</textarea>
                                                    @error('description')
                                                        <span class="text-danger">{{ $message }}</span>
                                                    @enderror
                                                </div>
